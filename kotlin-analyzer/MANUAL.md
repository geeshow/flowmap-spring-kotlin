...
```bash
cd kotlin-analyzer
gradle wrapper --gradle-version 8.12     # 최초 1회
...
node-link 형식. 키 순서·null 포함은 Python 도구 계약과 일치.
...
```jsonc
{
  "branch": "develop", "commitCount": 40, "depth": 3, "changedNodeCount": 50,
  "commits": [{
...
```
- `commits[]` — 커밋별 상세(변경노드 + 영향 엔드포인트/서비스).
